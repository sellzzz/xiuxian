...
`config/story-config.js` 的 `legacyBoons` 可覆盖轮回传承的 `name`、`desc` 与 `effects`；例如可调整“因果罗盘”的开局属性，而不需要改动核心循环。
...
聚焦选择按钮时会关联路线、条件和资源预览，键盘与屏幕阅读器也能获得完整决策信息。
结局页会额外统计本世获得的法宝数量与已回应的多年因果（0 / 3），方便比较不同轮回路线的收获。

卡牌上方的“如何修行”说明默认收起，首次进入时可快速查看左右选择、上滑打坐、键盘快捷键和第一幕限制。

顶部“轮回”状态会持续显示已完成世数、最高境界和结局收集进度，方便在每一局修行中追踪长期成长。收集五种不同结局后，还会解锁隐藏传承“因果罗盘”，下一世开局获得道心与宗门贡献优势。
