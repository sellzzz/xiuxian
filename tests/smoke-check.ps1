...
foreach ($marker in @('grantArtifact', 'setArtifactReward', 'artifactBonus', 'artifactEffectSummary', 'artifactRewards', 'artifactRewardText', 'configuredArtifactMarkup', 'artifactCollectionMarkup', 'artifactResonanceMarkup', 'artifactResonanceText', 'artifactActiveAffordable', 'activeArtifactResonances', 'artifactCatalog()', 'artifactCollector', 'requiresConfiguredArtifact', 'artifact-resonance-badge', 'artifact-cooldown', 'artifact-collection-summary', 'refreshArtifactInspector', 'announceArtifactResonances', 'announceArtifactResonanceLoss', 'cleanArtifactResonance', 'setArtifactResonance', 'removeArtifactResonance', 'resonances:artifactResonanceOverrides', 'resonances:activeArtifactResonances', 'removed={[id]:artifactResonanceOverrides[id]}', 'grantArtifact(id,refresh=true)', 'setArtifactResonance(id,resonance)', 'removeArtifactResonance(id)', 'migrateSave', 'rawState', 'Array.isArray(rawState)', 'forEach(key=>{if(!Array.isArray(saved[key]))saved[key]=[]})', 'exportSave', 'importSaveFile', 'importSaveInput', 'artifact-activation-toast', 'artifact-collection-summary', 'inspectArtifact', 'removeInspectedArtifact', 'useInspectedArtifact', 'showArtifactActivation', 'showArtifactAcquisition', 'achievement-toast', 'meditation-toast', 'aria-live', 'event.ctrlKey||event.metaKey', 'aria-disabled', 'formatChanges', 'projected===null', 'tickArtifactCooldowns', 'useArtifact(id)', 'artifactCooldowns?.[id]', 'active.label', 'setDragPreview', 'resetCard', 'Math.abs(dy)>=Math.abs(dx)', 'swipeHint.textContent=state.scene', 'onpointerenter=show', 'onfocus=show', 'SAVE_VERSION=5', 'LEGACY_MUSIC_VOLUME_KEY', 'music-volume-v2', 'realmLevel||0,realms.length-1')) {
