...
foreach ($marker in @('lifeResourceCap', "key==='life'", 'available=state.scene', 'formatYears(years)', 'choiceLifeCost', 'renderDecisionCostBadge', 'decision-cost-badge', 'showMeditationResult', 'meditation-result-toast', 'techniqueMasteryValue()', 'masteryText', 'lifeCostPreview', "state.life<choiceLifeCost()", 'refreshLifePillCopy', 'lifeResourceCap()}')) {
  if ($appSource.IndexOf($marker, [StringComparison]::Ordinal) -lt 0) {
