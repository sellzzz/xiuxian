...
foreach ($marker in @("requiresConfiguredArtifact:'heartMirror'", 'grantArtifact', 'setArtifactReward', 'artifactBonus', 'artifactEffectSummary', 'artifactRewards', 'artifactRewardText', 'configuredArtifactMarkup', 'artifactCollectionMarkup', 'artifactResonanceMarkup', 'artifactCatalogMarkup', 'artifact-catalog-item', 'artifact-resonance-requirements', 'artifactResonanceText', 'artifactActiveAffordable', 'activeArtifactResonances', 'artifactCatalog()', 'artifactCollector', 'requiresConfiguredArtifact', 'eventConditionOverrides', 'cleanEventCondition', 'artifactOverrides[value]', 'setEventCondition', 'removeEventCondition', 'configuredCondition', 'Object.keys(configuredCondition).length', 'queuedEvent', 'nextEventIndex', 'eventConditionOverrides[eventType]=cleanEventCondition(condition)', 'eventConditionHint', 'conditionHint', 'const condition={...event,...(eventConditionOverrides[event.type]||{})}', 'clampStateValue', 'claimQuestBase', 'hp:clampStateValue', 'state.realmLevel>=3', 'artifact-resonance-badge', 'artifact-cooldown', 'artifact-collection-summary', 'questStreak', 'questKeeper', 'nextStreakReward', 'currentEventConditions', 'eventConditionElement', 'worldCycleCatalog.find', 'requiresMaster', 'requiresEstate', 'requiresDemonVictory', 'diagnostics=function', 'showQuestReward', 'quest-reward-toast', 'refreshArtifactInspector', 'announceArtifactResonances', 'announceArtifactResonanceLoss', 'cleanArtifactResonance', 'setArtifactResonance', 'removeArtifactResonance', 'resonances:artifactResonanceOverrides', 'resonances:activeArtifactResonances', 'removed={[id]:artifactResonanceOverrides[id]}', 'grantArtifact(id,refresh=true)', 'setArtifactResonance(id,resonance)', 'removeArtifactResonance(id)', 'QingyunStoryAPI.useArtifact=function', 'migrateSave', 'rawState', 'Array.isArray(rawState)', 'forEach(key=>{if(!Array.isArray(saved[key]))saved[key]=[]})', 'exportSave', 'importSaveFile', 'importSaveInput', 'artifact-activation-toast', 'artifact-collection-summary', 'inspectArtifact', 'removeInspectedArtifact', 'useInspectedArtifact', 'showArtifactActivation', 'showArtifactAcquisition', 'achievement-toast', 'meditation-toast', 'showMeditationAnimation(gain=0,recovery=0)', 'resourceLabel(k)', 'showQuestReward', 'quest-reward-toast', 'aria-live', 'aria-valuetext', 'aria-valuenow', 'leftPreview', 'rightPreview', 'showActionResult', 'action-result-toast', 'showRewindAnimation', 'rewind-toast', 'event.ctrlKey||event.metaKey', 'aria-disabled', 'formatChanges', 'projected===null', 'tickArtifactCooldowns', 'artifactCooldowns?.[id]', 'active.label', 'setDragPreview', 'resetCard', 'Math.abs(dy)>=Math.abs(dx)', 'swipeHint.textContent=state.scene', 'realmLevel,realm:state?realms[realmLevel]', 'onpointerenter=show', 'onfocus=show', 'SAVE_VERSION=5', 'LEGACY_MUSIC_VOLUME_KEY', 'music-volume-v2', 'realmLevel||0,realms.length-1')) {
